...
interface MenuItem {
  ...
  mealPeriod: string | null; featured: boolean;
}
interface Category {
    ...
    <div className={`min-h-screen bg-stone-950 ${cartHasItems ? "pb-28" : "pb-10"}`}>
      ...
      <div className="relative overflow-hidden animate-fade-in">
        ...
        <div className="relative px-5 pt-8 pb-8 max-w-lg mx-auto">
          ...
          {/* Brand: logo o icona + nome ristorante */}
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-stone-400 text-sm mt-1">
                {orderingEnabled
                  ? "Sfoglia il menu e ordina direttamente dal tavolo"
                  : "Sfoglia il menu — chiedi al personale per ordinare"}
              </p>
            </div>
            ...
            {!orderingEnabled && (
              <span className="shrink-0 text-[10px] font-semibold uppercase tracking-wider bg-stone-800 text-stone-400 rounded-full px-3 py-1.5 mt-1">
                Solo menu
              </span>
            )}
          </div>
      ...
      {featuredItems.length > 0 && (
        <div
          className="max-w-lg mx-auto px-4 mt-4 animate-slide-in-up"
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 px-3 py-3">
                      <div className="w-0.5 self-stretch rounded-full bg-amber-500/60 shrink-0" />
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {numbered.length > 0 && (
                            <button onClick={() => toggleAllergenPanel(item.id)} className={`shrink-0 flex items-center gap-0.5 rounded-full px-1.5 py-0.5 transition-colors ${allergenOpen ? "bg-amber-500/20 text-amber-400" : "text-stone-500 hover:text-amber-400"}`}>
                      ...
                      {orderingEnabled && (
                        <div className="flex items-center gap-2 shrink-0">
                          {qty > 0 ? (
                            <>
                              <button onClick={() => removeFromCart(item.id)} className="h-7 w-7 rounded-full bg-stone-700 flex items-center justify-center hover:bg-stone-600 active:scale-90 transition-all"><Minus className="h-3 w-3 text-stone-200" /></button>
                              <span className="w-5 text-center font-bold text-stone-100 text-sm tabular-nums">{qty}</span>
                              <button onClick={() => addToCart(item)} className="h-7 w-7 rounded-full bg-amber-500 flex items-center justify-center hover:bg-amber-400 active:scale-90 transition-all"><Plus className="h-3 w-3 text-stone-950" /></button>
                            </>
                          ) : (
                            <button onClick={() => addToCart(item)} className="h-8 w-8 rounded-full bg-amber-500 flex items-center justify-center hover:bg-amber-400 active:scale-90 transition-all"><Plus className="h-3.5 w-3.5 text-stone-950" /></button>
                          )}
                        </div>
                      )}
                    </div>
                      ...
                      <div className="overflow-hidden min-h-0">
                        <div className="mx-3 mb-3 rounded-lg bg-stone-900 border border-amber-500/20 px-3 py-2.5">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-[10px] font-semibold uppercase tracking-wider text-amber-400/80 flex items-center gap-1"><AlertTriangle className="h-2.5 w-2.5" /> Allergeni</span>
            ...
            >
              ...
              >
                ...
                {orderingEnabled && catCartQty > 0 && (
                  <span className="shrink-0 bg-amber-500 text-stone-950 text-xs font-bold rounded-full px-2 py-0.5 animate-fade-in-scale">
                    {catCartQty}
                  </span>
                )}
                <ChevronDown
                  className={`shrink-0 h-4 w-4 text-stone-400 transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`}
                />
              </button>
              ...
              <div className={`grid transition-all duration-300 ease-in-out ${isOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
                <div className="overflow-hidden min-h-0">
                  <div className="px-3 pb-3 pt-1 space-y-2 border-t border-stone-800">
                    {availableItems.map((item, itemIdx) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="bg-stone-800/50 rounded-xl overflow-hidden"
                          style={isOpen ? { animation: `slide-in-up 0.3s cubic-bezier(0.16,1,0.3,1) both`, animationDelay: `${itemIdx * 0.04}s` } : undefined}
                        >
                          <div className="flex items-center gap-3 px-3 py-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {item.description && (
                                <p className="text-xs text-stone-500 leading-snug mt-0.5 line-clamp-2">{item.description}</p>
                              )}
                              <p className="text-amber-400 font-semibold text-sm mt-1">€{parseFloat(item.price).toFixed(2)}</p>
                            </div>
                            ...
                            <div className="overflow-hidden min-h-0">
                              <div className="mx-3 mb-3 rounded-lg bg-stone-900 border border-amber-500/20 px-3 py-2.5">
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <button onClick={() => setActiveAllergenItem(null)}>
                                    <X className="h-3 w-3 text-stone-500 hover:text-stone-300" />
                                  </button>
                                </div>
                                <div className="flex flex-wrap gap-1.5">
                                  {itemAllergens.map((a) => {
      ...
      {lightboxUrl && (
        ...
        >
          ...
          <img
            src={lightboxUrl}
            alt=""
            className="max-w-full max-h-[85vh] rounded-xl object-contain shadow-2xl animate-fade-in-scale"
